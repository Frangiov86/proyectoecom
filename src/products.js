export let products = [
  {
    id: "1",
    title: "Asus X515",
    price: 1250,
    description:
      "Notebook Asus X515, ideal para trabajar con la potencia necesaria",
    imageUrl:
      "https://res.cloudinary.com/deg6afpqs/image/upload/v1729992591/aSUS_x515_rgg79x.webp",
    stock: 5,
    category: "trabajo",
  },
  {
    id: "2",
    title: "Lenovo LOQ 15",
    price: 1550,
    stock: 8,
    description:
      "Liviana y poderosa. Con una NVIDIA RTX 4060 para trabajar cómodamente en tus proyectos",
    category: "trabajo",
    imageUrl:
      "https://res.cloudinary.com/deg6afpqs/image/upload/v1729992591/Lenovo_LOQ_15_bsoyvi.avif",
  },
  {
    id: "3",
    title: "Lenovo Legion Pro 5",
    price: 1854,
    stock: 5,
    description:
      "Gaming extremo, equilibrada y poderosa. Ideal para el ocio, diseño 3d y programación",
    category: "gamer",
    imageUrl:
      "https://res.cloudinary.com/deg6afpqs/image/upload/v1729992591/Lenovo_Legion_Pro_5_dyhetw.jpg",
  },
  {
    id: "4",
    title: "Asus Rog Strix G15",
    price: 2150,
    stock: 5,
    description:
      "Poderosa, de la gama Asus lo mas actual y mejor para lo que necesitas en cuanto experiencia gaming y diseño.",
    category: "gamer",
    imageUrl:
      "https://res.cloudinary.com/deg6afpqs/image/upload/v1729992591/Asus_Rog_hqabng.webp",
  },
];
